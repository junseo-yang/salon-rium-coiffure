...
export async function getImageURL(task_id: String) {
    ...
    const resp = await fetch(`${AI_LAB_QUERY_URL}?task_id=${task_id}`, {
        ...
        retryOn: async (attempt, error, response) => {
            ...
            if (json.error_detail.message) {
                throw new Error(json.error_detail.message);
            }
...
export async function request(formData: FormData) {
    ...
    if (data.message) {
        throw new Error(`${data.message}. Contact to Admin`);
    }
    ...
    if (!response.ok) {
        throw new Error(data.error_detail.message);
    }
