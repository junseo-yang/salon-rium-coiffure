...
test("sign in with google", async ({ page }) => {
    ...
    await expect(
        page.getByText(process.env.USERNAME_P as string)
    ).toBeVisible();
...
test("update staff", async ({ page }) => {
    ...
    await page.locator('li').filter({ hasText: "Test Staff" }).getByRole('button').first().click()
...
test("delete staff", async ({ page }) => {
    ...
    await page.locator('li').filter({ hasText: "John Doe" }).getByRole('button').nth(1).click()
